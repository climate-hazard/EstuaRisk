...
for i, feature in enumerate(json_data['features']):
    ...
    # elif isinstance(coords[0][1][0], float):
    #     (lat, lon) = transformer.transform(coords[0][1][0], coords[0][1][1])
    #     json_data['features'][i]['geometry']['coordinates'][0][1] = [lat, lon]
    else:   # Check is it ok with deeper nests? (Converted_Flood_Outlines_Blackwater.json)
        print('>>>>', coords[0][0][0])
