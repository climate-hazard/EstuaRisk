...
# Flood outlines downloaded from https://environment.data.gov.uk/DefraDataDownload/?mapService=EA/RecordedFloodOutlines&Mode=spatial
...
for i in range(len(df)):
    ...
    json_file = os.path.join('data', 'flood_outline',
            f'Recorded_Flood_Outlines_{name}.json')
    ...
    for i, feature in enumerate(json_data['features']):
        ...
        feature_coords = []
        if isinstance(coords[0][0][0], list):   # Check is it ok with deeper nests? (Converted_Flood_Outlines_Blackwater.json)
    ...
    with open(json_file_out, 'w') as outfile:
        content = json_template % features
